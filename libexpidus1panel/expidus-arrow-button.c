...
#include <libexpidus1panel/expidus-arrow-button.h>
#include <libexpidus1panel/libexpidus1panel-alias.h>



/**
 * SECTION: expidus-arrow-button
 * @title: ExpidusArrowButton
...
static void     expidus_arrow_button_set_property         (GObject               *object,
                                                        guint                  prop_id,
                                                        const GValue          *value,
                                                        GParamSpec            *pspec);
static void     expidus_arrow_button_get_property         (GObject               *object,
                                                        guint                  prop_id,
                                                        GValue                *value,
                                                        GParamSpec            *pspec);
static void     expidus_arrow_button_dispose              (GObject               *object);
static gboolean expidus_arrow_button_draw                 (GtkWidget             *widget,
                                                        cairo_t               *cr);
static void     expidus_arrow_button_get_preferred_width  (GtkWidget             *widget,
                                                        gint                  *minimum_width,
                                                        gint                  *natural_width);
static void     expidus_arrow_button_get_preferred_height (GtkWidget             *widget,
                                                        gint                  *minimum_height,
                                                        gint                  *natural_height);
static void     expidus_arrow_button_size_allocate        (GtkWidget             *widget,
                                                        GtkAllocation         *allocation);



struct _ExpidusArrowButtonPrivate
{
...
G_DEFINE_TYPE_WITH_PRIVATE (ExpidusArrowButton, expidus_arrow_button, GTK_TYPE_TOGGLE_BUTTON)



static void
expidus_arrow_button_class_init (ExpidusArrowButtonClass * klass)
{
  GObjectClass   *gobject_class;
  GtkWidgetClass *gtkwidget_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->get_property = expidus_arrow_button_get_property;
  gobject_class->set_property = expidus_arrow_button_set_property;
  gobject_class->dispose = expidus_arrow_button_dispose;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->draw = expidus_arrow_button_draw;
  gtkwidget_class->get_preferred_width = expidus_arrow_button_get_preferred_width;
  gtkwidget_class->get_preferred_height = expidus_arrow_button_get_preferred_height;
  gtkwidget_class->size_allocate = expidus_arrow_button_size_allocate;

  /**
   * ExpidusArrowButton::arrow-type-changed
  ...
  arrow_button_signals[ARROW_TYPE_CHANGED] =
      g_signal_new (g_intern_static_string ("arrow-type-changed"),
                    ...
                    G_SIGNAL_RUN_LAST,
                    G_STRUCT_OFFSET (ExpidusArrowButtonClass, arrow_type_changed),
                    NULL, NULL,
                    g_cclosure_marshal_VOID__ENUM,
                    G_TYPE_NONE, 1, GTK_TYPE_ARROW_TYPE);

  /**
   * ExpidusArrowButton:arrow-type
   *
...
static void
expidus_arrow_button_init (ExpidusArrowButton *button)
{
  GtkStyleContext *context;
  GtkCssProvider  *provider;

  button->priv = expidus_arrow_button_get_instance_private (button);
  ...
  gtk_css_provider_load_from_data (provider, ".expidus1-panel button { padding: 0; }", -1, NULL);
...
static void
expidus_arrow_button_set_property (GObject      *object,
                                guint         prop_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
  ExpidusArrowButton *button = EXPIDUS_ARROW_BUTTON (object);

  switch (prop_id)
    {
    case PROP_ARROW_TYPE:
      expidus_arrow_button_set_arrow_type (button, g_value_get_enum (value));
      break;
...
static void
expidus_arrow_button_get_property (GObject    *object,
                                guint       prop_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_ARROW_TYPE:
      g_value_set_enum (value, expidus_arrow_button_get_arrow_type (button));
      break;
...
static void
expidus_arrow_button_dispose (GObject *object)
{
  ExpidusArrowButton *button = EXPIDUS_ARROW_BUTTON (object);
  ...
  (*G_OBJECT_CLASS (expidus_arrow_button_parent_class)->dispose) (object);
}



static gboolean
expidus_arrow_button_draw (GtkWidget *widget,
                        cairo_t   *cr)
{
  ExpidusArrowButton *button = EXPIDUS_ARROW_BUTTON (widget);
  GtkWidget       *child;
  gdouble          x, y, width;
  GtkAllocation    alloc;
  gdouble          angle;
  ...
  /* draw the button */
  (*GTK_WIDGET_CLASS (expidus_arrow_button_parent_class)->draw) (widget, cr);
...
static void
expidus_arrow_button_get_preferred_width (GtkWidget *widget,
                                       gint      *minimum_width,
                                       gint      *natural_width)
{
  ExpidusArrowButton *button = EXPIDUS_ARROW_BUTTON (widget);
  ...
  if (child != NULL
      && gtk_widget_get_visible (child))
    {
      ...
      (*GTK_WIDGET_CLASS (expidus_arrow_button_parent_class)->get_preferred_width) (widget, &minimum_child_width, &natural_child_width);
...
static void
expidus_arrow_button_get_preferred_height (GtkWidget *widget,
                                        gint      *minimum_height,
                                        gint      *natural_height)
{
  ExpidusArrowButton *button = EXPIDUS_ARROW_BUTTON (widget);
  ...
  if (child != NULL
    ...
    {
      /* use gtk for the widget size */
      (*GTK_WIDGET_CLASS (expidus_arrow_button_parent_class)->get_preferred_height) (widget, &minimum_child_height, &natural_child_height);
...
static void
expidus_arrow_button_size_allocate (GtkWidget     *widget,
                                 GtkAllocation *allocation)
{
  ExpidusArrowButton *button = EXPIDUS_ARROW_BUTTON (widget);
  GtkWidget       *child;
  ...
  /* allocate the button */
  (*GTK_WIDGET_CLASS (expidus_arrow_button_parent_class)->size_allocate) (widget, allocation);
...
static gboolean
expidus_arrow_button_blinking_timeout (gpointer user_data)
{
  ExpidusArrowButton *button = EXPIDUS_ARROW_BUTTON (user_data);
...
static void
expidus_arrow_button_blinking_timeout_destroyed (gpointer user_data)
{
  ExpidusArrowButton *button = EXPIDUS_ARROW_BUTTON (user_data);
...
/**
 * expidus_arrow_button_new:
 * @arrow_type : #GtkArrowType for the arrow button
 *
 * Creates a new #ExpidusArrowButton widget.
 *
 * Returns: The newly created #ExpidusArrowButton widget.
 **/
GtkWidget *
expidus_arrow_button_new (GtkArrowType arrow_type)
{
  return g_object_new (EXPIDUS_TYPE_ARROW_BUTTON,
 ...
 **/
GtkArrowType
expidus_arrow_button_get_arrow_type (ExpidusArrowButton *button)
{
  g_return_val_if_fail (EXPIDUS_IS_ARROW_BUTTON (button), GTK_ARROW_UP);
 ...
 **/
void
expidus_arrow_button_set_arrow_type (ExpidusArrowButton *button,
                                  GtkArrowType     arrow_type)
{
  g_return_if_fail (EXPIDUS_IS_ARROW_BUTTON (button));
 ...
 **/
gboolean
expidus_arrow_button_get_blinking (ExpidusArrowButton *button)
{
  g_return_val_if_fail (EXPIDUS_IS_ARROW_BUTTON (button), FALSE);
 ...
 **/
void
expidus_arrow_button_set_blinking (ExpidusArrowButton *button,
                                gboolean         blinking)
{
  g_return_if_fail (EXPIDUS_IS_ARROW_BUTTON (button));
      ...
      if (button->priv->blinking_timeout_id == 0)
        {
          ...
          button->priv->blinking_timeout_id =
              gdk_threads_add_timeout_full (G_PRIORITY_DEFAULT_IDLE, 500,
                                            expidus_arrow_button_blinking_timeout, button,
                                            expidus_arrow_button_blinking_timeout_destroyed);
          expidus_arrow_button_blinking_timeout (button);
        }
    }
...
#define __EXPIDUS_ARROW_BUTTON_C__
