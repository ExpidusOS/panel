                             ...
                             gpointer channel)
{
  ...
  esconf_channel_set_string (ESCONF_CHANNEL (channel), prop, "actions");
}
                          ...
                          gpointer channel)
{
  ...
  if (esconf_channel_has_property (channel, str))
    return;

  /* set appearance to button mode */
  esconf_channel_set_uint (channel, str, 0);

  /* read and remove the old properties */
  g_snprintf (str, sizeof (str), "%s/first-action", prop);
  first_action_int = esconf_channel_get_uint (channel, str, 0) + 1;
  esconf_channel_reset_property (channel, str, FALSE);

  g_snprintf (str, sizeof (str), "%s/second-action", prop);
  second_action_int = esconf_channel_get_uint (channel, str, 0);
  esconf_channel_reset_property (channel, str, FALSE);
  ...
  esconf_channel_set_bool (channel, str, second_action_int > 0);
  ...
  esconf_channel_set_array (channel, str,
...
gboolean
migrate_config (EsconfChannel  *channel,
                gint            configver,
                GError        **error)
{
  GHashTable *plugins;
  ...
  plugins = esconf_channel_get_properties (channel, "/plugins");
