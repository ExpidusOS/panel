  ...
  if (strcmp (element_name, "channel") == 0)
    {
              ...
              if (strcmp (attribute_names[i], "name") == 0)
                {
                  ...
                  /* this is an expidus1-panel workaround to make it work
                   * with the custom channel names */
                  channel_name = EXPIDUS_PANEL_CHANNEL_NAME;
                }
            }
        }
      ...
      if (channel_name != NULL)
        {
          /* open the esconf channel */
          parser->channel = esconf_channel_get (channel_name);
        }
      else
        {
      ...
      if (parser->array != NULL)
        {
          ...
          esconf_channel_set_arrayv (parser->channel, prop_path, parser->array);
          g_free (prop_path);

          esconf_array_free (parser->array);
          parser->array = NULL;
        }
      ...
      if (prop_name != NULL && prop_type != NULL)
        {
          ...
          if (type == G_TYPE_BOXED)
          ...
          else if (type != G_TYPE_NONE && prop_value != NULL)
            {
              ...
              prop_path = migrate_default_property_path (parser);
              esconf_channel_set_property (parser->channel, prop_path, &value);
              g_free (prop_path);
